...
async def _generar_carta_porte_for_scope(payload: CartaPorteRequest, scope: dict):
    ...
    now = datetime.now(timezone.utc).isoformat()
    metadata = {
        "empresa_rfc": emisor.get("rfc") or "",
        "rfc_emisor": emisor.get("rfc") or "",
        "empresa_nombre": emisor.get("nombre") or "",
    ...
    factura_row = _scope_row(scope, {
        ...
        "empresa_nombre": emisor.get("nombre") or "",
        "cliente_nombre": receptor.get("nombre") or "",
        "fecha_emision": cfdi_fecha or None,
        "fecha_timbrado": pac_fecha_timbrado or now,
        "rfc_receptor": receptor["rfc"],
        "volumen_litros": litros,
        "importe": payload.importe,
