...
function renderCartaPorteWizard(){
  ...
  const helperRows = (CATALOGOS.ayudantes || []).map(a => `<label class="cp-helper-option"><input class="cp-helper-check" type="checkbox" value="${esc(a.id)}" onchange="updateCpHelperSummary();invalidateCpPreview()"><span><b>${esc(a.nombre || 'Ayudante')}</b>${a.rfc ? `<small>${esc(a.rfc)}</small>` : ''}</span></label>`).join('') || '<div class="cp-helper-empty">No hay ayudantes activos en el catálogo.</div>';
  host.innerHTML = `
    <style>
    ...
    <div class="cp-wizard">
      ...
      <div class="cp-step">
        ...
        <div class="form-grid">
          ...
          <div><label>Ayudantes (opcional)</label><details id="cpAyudantes" class="cp-helper-dropdown"><summary id="cpAyudantesSummary">Selecciona ayudantes</summary><div class="cp-helper-menu">${helperRows}</div></details><div class="muted" style="font-size:12px;margin-top:4px">Puedes seleccionar varios del catálogo. Se agregan como figuras SAT 04 Notificado.</div></div>
        </div>
      </div>
      <div class="cp-step">
        <h3>3. Mercancía</h3>
...
function applyCpRutaDefaults(){
  ...
  document.querySelectorAll('.cp-helper-check').forEach(input => { input.checked = false; });
  updateCpHelperSummary();
  updateCpLlegada();
  updateCpPeso();
...
function selectedCp(){
  ...
  const ayudanteIds = Array.from(document.querySelectorAll('.cp-helper-check:checked')).map(input => String(input.value));
  const ayudantes = (CATALOGOS.ayudantes || []).filter(a => ayudanteIds.includes(String(a.id)));
  ...
  return {merc, veh, chofer, ayudantes, origen, destino, litrosNum, peso};
}
function updateCpHelperSummary(){
  const summary = document.getElementById('cpAyudantesSummary');
  if(!summary) return;
  const selected = Array.from(document.querySelectorAll('.cp-helper-check:checked'));
  if(!selected.length){ summary.textContent = 'Selecciona ayudantes'; return; }
  const names = selected.map(input => (CATALOGOS.ayudantes || []).find(a => String(a.id) === String(input.value))?.nombre || 'Ayudante');
  summary.textContent = names.length === 1 ? names[0] : `${names.length} ayudantes seleccionados`;
}
...
function prepararCartaPortePreview(){
  ...
  const html = [
    ...
    ['Ayudantes', s.ayudantes.length ? s.ayudantes.map(a => a.nombre).join(', ') : 'Sin ayudantes'],
...
function cartaPortePayload(){
  ...
  return {
    ...
    ayudante_ids: s.ayudantes.map(a => numericId(a.id)).filter(Boolean),
    ...
    <div class="acp-modal-layer" id="cpConfirmModal">
      <div class="acp-modal">
        ...
        <div class="cp-confirm-list">
          ...
          <div><span>Ayudantes</span><b>${esc(s.ayudantes.length ? s.ayudantes.map(a => a.nombre).join(', ') : 'Sin ayudantes')}</b></div>
...
function acpCfg(kind){
  return {
    ...
    ayudantes:{label:'Ayudantes',empty:'Agrega ayudantes opcionales para Carta Porte'},
...
function acpTitle(kind,row){
  ...
  if(kind==='ayudantes') return row.nombre || 'Ayudante';
...
async function loadAssistantCpPostalLookup(){
  ...
  if(!assistantCpPostalLookupPromise){
    assistantCpPostalLookupPromise = api('/api/internal-auth/gas-lp/catalogos-postales')
      .then(data => {
...
function renderAssistantCpCatalogs(){
  ...
  host.innerHTML = `
    ...
    <div class="acp-head"><div><h2>Configuración Carta Porte</h2><p class="muted" style="margin:4px 0 0">Vehículos, choferes, ayudantes, mercancías, rutas e instalaciones de Administración habilitadas para Carta Porte.</p></div><button class="btn ghost" type="button" onclick="loadCatalogos()"><i class="fa-solid fa-arrows-rotate"></i> Actualizar</button></div>
    <div class="acp-tabs">${['vehiculos','choferes','ayudantes','instalaciones','mercancias','rutas'].map(k=>`<button class="${k===assistantCpKind?'active':''}" type="button" data-acp-action="switch-tab" data-acp-catalog="${esc(k)}">${acpCfg(k).label}</button>`).join('')}</div>
...
function renderAssistantCpForm(){
  ...
  if(kind==='mercancias') body = [
    ...
    acpField('acpm_peligro','Material peligroso','Sí','text','readonly class="locked-field"','Gas LP se clasifica siempre como material peligroso.'),
    acpSelect('acpm_clavep','<span class="acp-required">Clave material peligroso SAT</span>','<option value="1075">1075 · Gases de petróleo licuados (Gas LP)</option>',row?.clave_material_peligroso||'1075','Catálogo SAT/NOM aplicable a la mercancía Gas LP.'),
  ...
  const icon = kind==='vehiculos' ? 'fa-truck' : ['choferes','ayudantes'].includes(kind) ? 'fa-id-card' : kind==='rutas' ? 'fa-route' : kind==='mercancias' ? 'fa-boxes-stacked' : 'fa-location-dot';
...
function validateAssistantCp(kind){
  ...
  if(kind==='mercancias'){ req('clave material peligroso', acpm_clavep.value); req('embalaje SAT', acpm_emb.value); }
