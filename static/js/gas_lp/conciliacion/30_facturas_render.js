...
function facturaEstadoHtml(f){const state=facturaPagoState(f);return `<div class="state-stack" ${state.detail?`title="${esc(state.detail)}"`:''}>${state.badges.join('')}</div>`}
function complementoDocs(c){const perfil=activePerfilId?`&perfil_id=${encodeURIComponent(activePerfilId)}`:'';const q=`token=${encodeURIComponent(token)}${perfil}`;const id=encodeURIComponent(c.id||'');const cancelado=String(c.status||'').toLowerCase().includes('cancel');return `<div class="doc-actions icon-docs"><a class="btn ghost sm doc-icon" title="Ver PDF complemento" aria-label="Ver PDF complemento" target="_blank" href="/api/internal-auth/gas-lp/complementos-pago/${id}/pdf?${q}"><i class="fa-solid fa-file-invoice"></i><span class="sr-only">PDF</span></a><a class="btn ghost sm doc-icon" title="Descargar XML complemento" aria-label="Descargar XML complemento" target="_blank" href="/api/internal-auth/gas-lp/complementos-pago/${id}/xml?${q}"><i class="fa-solid fa-receipt"></i><span class="sr-only">XML</span></a><button class="btn ghost sm doc-icon" title="Reenviar correo" aria-label="Reenviar correo" onclick="reenviarComplementoEmail('${esc(String(c.id||''))}')"><i class="fa-solid fa-envelope"></i><span class="sr-only">Correo</span></button>${cancelado?'':`<button class="btn danger sm doc-icon" title="Cancelar complemento" aria-label="Cancelar complemento" onclick="openCancelComplemento(${Number(c.id)})"><i class="fa-solid fa-ban"></i><span class="sr-only">Cancelar</span></button>`}</div>`}
...
function complementoFolio(c){const md=c.metadata||{};const serie=String(c.serie||md.serie||'P').trim();const raw=String(c.folio_usuario||md.folio_usuario||'').trim();const folio=/^\d{14}$/.test(raw)?String(Number(raw.slice(-6))):String(Number(raw)||raw);return folio?`${serie}-${folio}`:serie}
function renderFacturas(r){facturasRows.innerHTML=r.length?r.map(f=>{if(f.__kind==='complemento'){const rel=complementoFacturaShort(f);return `<tr><td class="nowrap date-col">${esc(dateDMY(f.fecha_timbrado||f.fecha_pago))}</td><td class="tipo-col"><span class="op-tag">Complemento</span><span class="cell-sub" title="${esc(rel)}">${esc(rel)}</span></td><td class="client-cell"><span class="cell-main" title="${esc(f.cliente||'Cliente')}">${esc(f.cliente||'Cliente')}</span><span class="cell-sub">${esc(f.rfc_receptor||'')}</span></td><td class="folio">${esc(complementoFolio(f))}</td><td class="uuid">${uuidHtml(f.uuid_sat)}</td><td class="route-cell"><span class="cell-main">Complemento</span></td><td class="num liters-col">—</td><td class="num money-col">${money(f.monto)}</td><td class="num iva-col">—</td><td class="num money-col"><b>${money(f.monto)}</b></td><td class="forma-col">${esc(f.forma_pago||'—')}</td><td class="metodo-col">Complemento</td><td class="user-col">${esc(f.realizado_por||'Sistema')}</td><td class="estado-col"><span class="pill">Complemento</span><span class="cell-sub">${esc(f.email_status||'Pendiente')}</span></td><td class="bank-col"><span class="cell-sub">—</span></td><td class="bank-actions-col"><span class="cell-sub">—</span></td><td class="docs-col">${complementoDocs(f)}</td></tr>`}return `<tr><td class="nowrap date-col">${esc(dateDMY(facturaDateKey(f)))}</td><td class="tipo-col"><span class="op-tag ${isTransfer(f)?'transfer':''}">${isTransfer(f)?'Traspaso':'Factura'}</span><span class="cell-sub">${esc(folio(f))}</span></td><td class="client-cell">${facturaClienteHtml(f)}</td><td class="folio">${esc(folio(f))}</td><td class="uuid">${uuidHtml(f.uuid_sat)}</td><td class="route-cell">${facturaInstalacionHtml(f)}</td><td class="num liters-col">${litros(f).toLocaleString('es-MX',{minimumFractionDigits:4,maximumFractionDigits:4})}</td><td class="num money-col">${money(subtotal(f))}</td><td class="num iva-col">${money(iva(f))}</td><td class="num money-col"><b>${money(total(f))}</b></td><td class="forma-col">${isTransfer(f)?'Traslado':esc(forma(f)||'—')}</td><td class="metodo-col">${esc(metodo(f))}</td><td class="user-col">${esc(realizadoPor(f))}</td><td class="estado-col">${facturaEstadoHtml(f)}</td><td class="bank-col">${bankStatusHtml(f)}</td><td class="bank-actions-col">${bankActionButton(f)}</td><td class="docs-col">${docs(f)}</td></tr>`}).join(''):'<tr><td colspan="17">Sin documentos para los filtros seleccionados.</td></tr>'}
...
function facturaClienteKey(f){return String(f.rfc_receptor||f.cliente||razon(f)||'SIN RFC').trim().toUpperCase()}
function facturaClienteLabel(f){const name=String(f.cliente||razon(f)||'Cliente').trim();const rfc=String(f.rfc_receptor||'').trim().toUpperCase();return `${name}${rfc?' · '+rfc:''}`}
function renderFacturaClientOptions(){
  if(!window.clienteFiltro)return;
  const previous=String(clienteFiltro.value||'').trim().toUpperCase();
  const byClient=new Map();
  fiscalDocumentRows().forEach(f=>{
    const key=facturaClienteKey(f);
    if(!byClient.has(key))byClient.set(key,facturaClienteLabel(f));
  });
  const options=[...byClient.entries()].sort((a,b)=>a[1].localeCompare(b[1],'es'));
  clienteFiltro.innerHTML='<option value="">Todos los clientes</option>'+options.map(([key,label])=>`<option value="${esc(key)}">${esc(label)}</option>`).join('');
  clienteFiltro.value=options.some(([key])=>key===previous)?previous:'';
}
...
function renderComplementos(r){if(!COMPLEMENTOS_SEARCHED){complementosRows.innerHTML='<tr><td colspan="8">Presiona Buscar PPD pendientes.</td></tr>';refreshSel();return}Object.keys(SEL).forEach(id=>{const f=PPD_PENDIENTES.find(x=>String(x.id)===String(id));if(!f||metodo(f)!=='PPD'||paid(f)||isCancel(f))delete SEL[id]});const selected=String(compClienteFiltro?.value||'').trim().toUpperCase();const ppd=r.filter(f=>metodo(f)==='PPD'&&!paid(f)&&!isCancel(f)&&(!selected||complementoClienteKey(f)===selected));complementosRows.innerHTML=ppd.length?ppd.map(f=>`<tr><td><input type="checkbox" ${SEL[f.id]?'checked':''} onchange="toggleSel(${Number(f.id)},this.checked)"></td><td>${esc(dateDMY(facturaDateKey(f)))}</td><td>${esc(razon(f))}<br><span class="cell-sub">${esc(f.rfc_receptor||'')}</span></td><td><b title="${esc(f.uuid_sat||'')}">${esc(folio(f))}</b></td><td class="num">${money(total(f))}</td><td class="num"><b class="err">${money(saldo(f))}</b></td><td><span class="cell-sub">${SEL[f.id]?'Capturar en validación':'Selecciona para validar'}</span></td><td>${esc(realizadoPor(f))}</td></tr>`).join(''):'<tr><td colspan="8">Sin facturas PPD pendientes.</td></tr>';refreshSel()}
