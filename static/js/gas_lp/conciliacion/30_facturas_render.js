...
function renderFacturas(r){facturasRows.innerHTML=r.length?r.map(f=>{if(f.__kind==='complemento'){const rel=complementoFacturaShort(f);return `<tr><td class="nowrap date-col">${esc(dateDMY(f.fecha_timbrado||f.fecha_pago))}</td><td class="tipo-col"><span class="op-tag" title="${esc(rel)}">Complemento</span></td><td class="client-cell"><span class="cell-main" title="${esc(f.cliente||'Cliente')}">${esc(f.cliente||'Cliente')}</span><span class="cell-sub">${esc(f.rfc_receptor||'')}</span></td><td class="folio">${esc(complementoFolio(f))}</td><td class="uuid">${uuidHtml(f.uuid_sat)}</td><td class="route-cell"><span class="cell-main">Complemento</span></td><td class="num liters-col">—</td><td class="num money-col">${money(f.monto)}</td><td class="num iva-col">—</td><td class="num money-col"><b>${money(f.monto)}</b></td><td class="forma-col">${esc(f.forma_pago||'—')}</td><td class="metodo-col">Complemento</td><td class="user-col">${esc(f.realizado_por||'Sistema')}</td><td class="estado-col"><span class="pill">Complemento</span><span class="cell-sub">${esc(f.email_status||'Pendiente')}</span></td><td class="bank-col"><span class="cell-sub">—</span></td><td class="bank-actions-col"><span class="cell-sub">—</span></td><td class="docs-col">${complementoDocs(f)}</td></tr>`}return `<tr><td class="nowrap date-col">${esc(dateDMY(facturaDateKey(f)))}</td><td class="tipo-col"><span class="op-tag ${isTransfer(f)?'transfer':''}">${isTransfer(f)?'Traspaso':'Factura'}</span></td><td class="client-cell">${facturaClienteHtml(f)}</td><td class="folio">${esc(folio(f))}</td><td class="uuid">${uuidHtml(f.uuid_sat)}</td><td class="route-cell">${facturaInstalacionHtml(f)}</td><td class="num liters-col">${litros(f).toLocaleString('es-MX',{minimumFractionDigits:4,maximumFractionDigits:4})}</td><td class="num money-col">${money(subtotal(f))}</td><td class="num iva-col">${money(iva(f))}</td><td class="num money-col"><b>${money(total(f))}</b></td><td class="forma-col">${isTransfer(f)?'Traslado':esc(forma(f)||'—')}</td><td class="metodo-col">${esc(metodo(f))}</td><td class="user-col">${esc(realizadoPor(f))}</td><td class="estado-col">${facturaEstadoHtml(f)}</td><td class="bank-col">${bankStatusHtml(f)}</td><td class="bank-actions-col">${bankActionButton(f)}</td><td class="docs-col">${docs(f)}</td></tr>`}).join(''):'<tr><td colspan="17">Sin documentos para los filtros seleccionados.</td></tr>'}
