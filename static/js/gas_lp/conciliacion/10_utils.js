...
const CONCILIACION_WORKSPACE_BY_TAB={dashboard:'resumen',facturas:'facturacion',publico:'facturacion',complementos:'facturacion',credito:'control',descuentos:'control',sat:'control'};
const CONCILIACION_DEFAULT_TAB={resumen:'dashboard',facturacion:'facturas',control:'credito'};
...
function switchTab(t){const workspace=CONCILIACION_WORKSPACE_BY_TAB[t]||'resumen';CONCILIACION_LAST_TAB[workspace]=t;document.querySelectorAll('.workspace-tab').forEach(b=>b.classList.toggle('active',b.dataset.workspace===workspace));document.querySelectorAll('.workspace-subtabs').forEach(n=>n.classList.toggle('hidden',n.dataset.subtabs!==workspace));document.querySelectorAll('.tab').forEach(b=>b.classList.toggle('active',b.dataset.tab===t));document.querySelectorAll('.section').forEach(s=>s.classList.toggle('active',s.dataset.section===t))}
