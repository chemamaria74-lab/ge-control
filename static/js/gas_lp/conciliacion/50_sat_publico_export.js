...
async function logout(){await fetch('/api/internal-auth/logout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token})}).catch(()=>{});localStorage.removeItem(CONCILIACION_TOKEN_KEY);location.href='/gas-lp/conciliacion'}
function changeSupervisionSpace(area='contable'){['ge_gaslp_conciliacion_token','sat_token','zc_token','sat_user_id','sat_email','sat_role','sat_assigned_perfil_id'].forEach(key=>localStorage.removeItem(key));['ge_flotilla_access','ge_flotilla_expires_at','ge_flotilla_identity'].forEach(key=>sessionStorage.removeItem(key));location.replace('/gas-lp/conciliacion?area='+encodeURIComponent(area))}
