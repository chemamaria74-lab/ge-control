(function(){
  ...
  function renderActivityCalendar(calendar){
    const days=calendar.days||[],drivers=calendar.drivers||[],host=$('activityCalendar');
    if(!days.length||!drivers.length){host.innerHTML='<div class="empty">No hay trayectos con chofer suficientes para construir la semana.</div>';return;}
    ...
    const previousStops=(driver,day)=>{const index=countedDays.indexOf(day);for(let offset=index-1;offset>=0;offset-=1){const record=driver.days?.[countedDays[offset]]||{};if(record.observed===true&&Number(record.stops||0)>0)return Number(record.stops);}return null;};
    const dayCellContent=(driver,day,record,symbol,rest,observed)=>{
      if(rest||!observed)return `<b>${symbol}</b>`;
      const distance=Number(record.distance_km||0),stops=Number(record.stops||0),vehicles=record.vehicle_numbers||[],baseline=driverStopBaseline(driver),ratio=baseline?stops/baseline:1,stopTone=ratio<.5?'critical':ratio<.8?'warning':'normal',previous=previousStops(driver,day),arrow=previous==null?'':stops>previous?'↑':stops<previous?'↓':'→',intensity=Math.max(4,Math.min(100,Math.round(ratio*100))),vehicleLabel=vehicles.length>1?`${vehicles.length} unidades`:(vehicles[0]||'Sin unidad');
      return `<b>${symbol}</b><span>${fmt(distance)} km</span><small class="activity-stops ${stopTone}" title="Ritmo habitual: ${fmt(baseline)} paradas">${fmt(stops)} par. ${arrow}</small><small class="activity-vehicle"><i class="fa-solid fa-truck"></i> ${esc(vehicleLabel)}</small>${vehicles.length>1?'<small class="activity-multi">Ver detalle</small>':''}<i class="activity-load ${stopTone}"><u style="width:${intensity}%"></u></i>`;
    };
    const showDayDetail=(driver,day)=>{
      const record=driver.days?.[day]||{},details=[...(record.trip_details||[])].sort((a,b)=>String(a.started_at||'').localeCompare(String(b.started_at||''))),distance=Number(record.distance_km||0),stops=Number(record.stops||0),trips=Number(record.trips||0),minutes=Number(record.drive_minutes||0),panel=$('activityDayDetail'),fullDay=new Intl.DateTimeFormat('es-MX',{day:'numeric',month:'long',year:'numeric',timeZone:'UTC'}).format(new Date(`${day}T12:00:00Z`));
      panel.hidden=false;
      panel.innerHTML=`<div class="activity-detail-head"><div><span>Desglose del día</span><h4>${esc(driver.driver_name||'Sin chofer identificado')} · ${esc(fullDay)}</h4><small>${fmt((record.vehicle_numbers||[]).length)} unidad${(record.vehicle_numbers||[]).length===1?'':'es'}</small></div><button type="button" class="btn secondary" data-close-activity-detail>Cerrar</button></div><div class="activity-detail-kpis"><div><span>Kilómetros</span><strong>${fmt(distance)} km</strong></div><div><span>Recorridos</span><strong>${fmt(trips)}</strong></div><div><span>Paradas</span><strong>${fmt(stops)}</strong></div><div><span>Conducción</span><strong>${duration(minutes)}</strong></div></div>${details.length?`<div class="activity-detail-table"><table><thead><tr><th>#</th><th>Horario</th><th>Unidad</th><th>Origen → destino</th><th>Km</th><th>Duración</th></tr></thead><tbody>${details.map((trip,index)=>`<tr><td>${index+1}</td><td>${clock(trip.started_at)}–${clock(trip.ended_at)}</td><td>${esc(trip.vehicle_number||'Sin unidad')}</td><td>${esc(trip.origin||'Origen no disponible')} → ${esc(trip.destination||'Destino no disponible')}</td><td>${fmt(trip.distance_km)} km</td><td>${duration(trip.duration_minutes)}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">No hay recorridos detallados para este día.</div>'}`;
