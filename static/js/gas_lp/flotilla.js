(function(){
  ...
  function renderActivityCalendar(calendar){
    ...
    const unitMaxDistance=unit=>Math.max(...countedDays.map(day=>Number(unit.days?.[day]?.distance_km||0)),1);
    const dayCellContent=(record,symbol,rest,observed,maxDistance)=>{
      if(rest||!observed)return `<b>${symbol}</b>`;
      const distance=Number(record.distance_km||0),stops=Number(record.stops||0),intensity=Math.max(4,Math.round(distance/maxDistance*100));
      return `<b>${symbol}</b><span>${fmt(distance)} km</span><small>${fmt(stops)} par.</small><i class="activity-load"><u style="width:${intensity}%"></u></i>`;
    };
